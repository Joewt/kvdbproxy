...
func Paginate(
	...
	if len(key) != 0 {
		iterator, err := getIterator(prefixStore, key, reverse)
		if err != nil {
			return nil, err
		}
	...
	iterator, err := getIterator(prefixStore, nil, reverse)
	if err != nil {
		return nil, err
	}
	defer iterator.Close()
...
func getIterator(prefixStore db.DB, start []byte, reverse bool) (db.Iterator, error) {
	if reverse {
		var end []byte
		if start != nil {
